 ...
 * Devuelve una promesa con el buffer de audio MP3 completo.
 */
function synthesizeEdgeTts(text, voice = 'ja-JP-NanamiNeural', rate = 1.0) {
  return new Promise((resolve, reject) => {
    ...
    ws.on('open', () => {
      ...
      // 2. Enviar petición SSML (Edge usa nombres de voz completos, ej. Microsoft Server Speech Text to Speech Voice (ja-JP, NanamiNeural))
    ...
    ws.on('close', () => {
      if (isTerminated && audioBuffers.length > 0) {
        resolve(Buffer.concat(audioBuffers));
      } else {
        reject(new Error('Conexión con Edge TTS cerrada de forma inesperada o sin datos.'));
      }
    });
